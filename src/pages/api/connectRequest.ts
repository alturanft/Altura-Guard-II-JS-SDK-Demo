...
console.log("test" ,process.env.ALTURA_API_KEY)
...
type AlturaGuardStore = {
  ...
  const useAlturaGuardStore = create<AlturaGuardStore>((set: SetState<AlturaGuardStore>) => ({
    alturaGuard: null,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'connect') {
      if (!guardCode) {
        return res.status(400).json({ error: 'Please enter Altura guard code.' });
      }
      ...
      try {
        ...
        console.error(error);
        return res.status(500).json({ error: 'Failed to connect to Altura Guard' });
      }
    } else if (action === 'signMessage') {
    const alturaGuard = useAlturaGuardStore.getState().alturaGuard;
      if (!alturaGuard) {
        return res.status(400).json({ error: 'Please connect to Altura Guard first.' });
      }
      ...
      try {
        ...
        if(signMessageCall.error){
            return res
            .status(400)
            .json({ error: signMessageCall.error });
        }
        ...
        return res
          .status(500)
          .json({ error: `Failed to sign message. Error: ${JSON.stringify(error)}` });
      }
        ...
        if(sendNativeTokenCall.error){
            return res
            .status(400)
            .json({ error: sendNativeTokenCall.error });
        }
          ...
          return res
            .status(500)
            .json({ error: `Failed to send Native Token. Error: ${JSON.stringify(error)}` });
        }
        ...
        if(sendContractTransactionCall.error){
          ...
          return res
            .status(500)
            .json({ error: `Failed to send contract call. Error: ${JSON.stringify(error)}` });
        }
